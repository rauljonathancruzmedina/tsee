			...
			if(count($tickets)== 0){
		 	...
		 	foreach ($tickets as $key => $value) {
		 		...
		 		if($_GET["tipo"] == "recibidos" && $value["tipo"] == "enviado" || $value["tipo"] == "papelera"){
			 		...
			 		$usuario = ControladorUsuarios::ctrMostrarUsuarios("id", $value["remitente"]);

			 	}

			 	if($_GET["tipo"] == "enviados" && $value["tipo"] == "enviado" || $value["tipo"] == "papelera"){

			 		$usuario = ControladorUsuarios::ctrMostrarUsuarios("id", $value["receptor"]);
				...
				if($_GET["tipo"] == "papelera" && $value["tipo"] == "papelera" && count($papelera) != 0){
					...
					if(count($papelera) == 1){
			 			...
			 			if($papelera[0] == $_GET["id_usuario"]){
					 		...
					 		$remitente = ControladorUsuarios::ctrMostrarUsuarios("id", $value["remitente"]);

					 		$receptor = ControladorUsuarios::ctrMostrarUsuarios("id", $value["receptor"]);
					 	}
			 	...
			 	$asunto = "<a href='index.php?ruta=soporte&soporte=lectura-msg&tipo=".$_GET["tipo"]."&idMsj=".$value["id_soporte"]."'>".substr($value["asunto"], 0, 30)."...</a>";

			 	if($value["adjuntos"] != "[]"){

			 		$adjuntos = "<i class='fa fa-paperclip'></i>";
